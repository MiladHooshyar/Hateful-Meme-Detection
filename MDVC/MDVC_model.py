...
class Model(nn.Module):
    ...
    def train_manual(self, criterion, optimizer, N_Epoch,
                     X_encoder, X_decoder, Label,
                     X_encoder_v, X_decoder_v, Label_v):
        for epoch in range(N_Epoch):
            running_loss = 0.0
            i = - 1
            for x_encoder, x_decoder, label in zip(X_encoder, X_decoder, Label):
                i += 1
                optimizer.zero_grad()
            ...
            outputs = self.forward(x_encode=X_encoder_v, x_decode=X_decoder_v,
                                   src_mask=None, trg_mask=None)
            loss_v = criterion(outputs[:, 0, :], Label_v)
            auc_v = roc_auc_score(Label_v.detach().numpy(), outputs[:, 0, 1].detach().numpy())
            print('[%d] loss: %.3f  val loss: %.3f  val auc: %.3f' %
                  (epoch + 1, running_loss / i, loss_v, auc_v))
